...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 72, 166, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 72, 194, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 611, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 649, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 699, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 738, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 790, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 831, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 883, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 925, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 976, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1019, MyFile)
!
      RETURN
...
!
      IF (Lwclock) THEN
        CALL wclock_on (ng, model, 64, 1070, MyFile)
      END IF
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
...
!
      IF (Lwclock) THEN
        CALL wclock_off (ng, model, 64, 1110, MyFile)
      END IF
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1161, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1200, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1251, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1292, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1342, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1380, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1430, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1469, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1520, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1561, MyFile)
!
      RETURN
...
!
      IF (Lwclock) THEN
        CALL wclock_on (ng, model, 64, 1611, MyFile)
      END IF
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
...
!
      IF (Lwclock) THEN
        CALL wclock_off (ng, model, 64, 1651, MyFile)
      END IF
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1701, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1741, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1791, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1832, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1882, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1924, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1975, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 2104, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 68, 2177, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 68, 2279, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2346, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2508, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2588, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2764, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2844, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 3021, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 3097, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 3259, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 3338, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 3515, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 69, 3590, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 69, 3734, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 69, 3801, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 69, 3945, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4045, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 4334, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4829, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 5142, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 5621, MyFile)
!
!-----------------------------------------------------------------------
!  Collect data from all nodes.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 5704, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 5780, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 5909, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 5994, MyFile)
!
!-----------------------------------------------------------------------
!  Read requested NetCDF file and scatter it to all nodes.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 6133, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 6216, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 6351, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 6436, MyFile)
!
!-----------------------------------------------------------------------
!  Collect and write data into requested NetCDF file.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 6581, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 6798, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 7273, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 7402, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 7468, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 7612, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 7676, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 7744, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 7840, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 8131, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 8607, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 8921, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 9401, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 9498, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 71, 9567, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 71, 9698, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 71, 9774, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 71, 9911, MyFile)
!
      RETURN
