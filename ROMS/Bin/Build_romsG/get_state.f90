...
!
      IF (((model.eq.0).or.(model.eq.iNLM)).and.(nrrec(ng).ne.0)) THEN
        ...
        IF (FoundError(exit_flag, NoError, 341, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                          OCEAN(ng) % zeta(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 665, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % ubar(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 792, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % umask,                         &
     &                        OCEAN(ng) % rubar)
            IF (FoundError(status, nf90_noerr, 845, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % vbar(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 919, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % v(:,:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 1165, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % vmask,                         &
     &                        OCEAN(ng) % rv)
            IF (FoundError(status, nf90_noerr, 1218, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                            OCEAN(ng) % t(:,:,:,Tindex,itrc))
              END IF
              IF (FoundError(status, nf90_noerr, 1289, MyFile)) THEN
                IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % AKt(:,:,:,itemp))
            IF (FoundError(status, nf90_noerr, 1405, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % gls)
            IF (FoundError(status, nf90_noerr, 1749, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % Akk)
            IF (FoundError(status, nf90_noerr, 1855, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % Akp)
            IF (FoundError(status, nf90_noerr, 1909, MyFile)) THEN
              IF (Master) THEN
...
!
      CALL wclock_off (ng, IDmod, 80, 7888, MyFile)
!
