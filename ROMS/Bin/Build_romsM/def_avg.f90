...
!
     ...
     &                 SetParAccess = .TRUE.)
        IF (FoundError(exit_flag, NoError, 515, MyFile)) RETURN
!
!  Define free-surface.
!
...
!
     ...
     &                   NF_FOUT, nvd3, v2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 615, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, t3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 981, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, p3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1084, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, p3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1103, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1142, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, v3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1258, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, v2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1756, MyFile)) RETURN
        END IF
!
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1869, MyFile)) THEN
...
!
     ...
     &                         ncid = AVG(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1878, MyFile)) RETURN
!
...
!
        CALL bbl_def_nf90 (ng, model, ldef, Aout, AVG)
        IF (FoundError(exit_flag, NoError, 2620, MyFile)) RETURN
!
