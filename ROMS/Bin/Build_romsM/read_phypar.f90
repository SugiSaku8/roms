...
!
     ...
     &                      Ngrids, Nfiles, inp_lib, GRD)
            CASE ('GRXNAME')
              ...
              Npts=load_s1d(Nval, Cval, Cdim, line, label, igrid,       &
     &                      Ngrids, Nfiles, io_nf90, GRX)
            CASE ('ININAME')
              label='INI - nonlinear model initial conditions'
              Npts=load_s1d(Nval, Cval, Cdim, line, label, igrid,       &
     &                      Ngrids, Nfiles, io_nf90, INI)
            CASE ('IRPNAME')
     ...
     &                      inp_lib, FRC)
            ...
            CASE ('VEGNAME', 'VEGNAM')
              DO i=1,LEN(vegname)
                vegname(i:i)=blank
              END DO
              vegname=TRIM(ADJUSTL(Cval(Nval)))
            CASE ('USRNAME')
              DO i=1,LEN(USRname)
                USRname(i:i)=blank
              END DO
          ...
          END SELECT
          IF (FoundError(exit_flag, NoError, 4489, MyFile)) RETURN
        END IF
      END DO
...
!
     ...
     &                      TRIM(DIA(ng)%head)
        ...
        IF (.not.find_file(ng, out, fname, 'GRDNAME')) THEN
          IF (FoundError(exit_flag, NoError, 7764, MyFile)) RETURN
        ELSE
     ...
     &      '                 Input Grid File:  ', TRIM(fname)
        ...
        IF (.not.find_file(ng, out, fname, 'ININAME')) THEN
          IF (FoundError(exit_flag, NoError, 7785, MyFile)) RETURN
        ELSE
          IF (Master.and.Lwrite) WRITE (out,230)                        &
        ...
        IF (LuvSrc(ng).or.LwSrc(ng).or.(ANY(LtracerSrc(:,ng)))) THEN
          ...
          IF (.not.find_file(ng, out, fname, 'SSFNAME')) THEN
            IF (FoundError(exit_flag, NoError, 7842, MyFile)) RETURN
          ELSE
            IF (Master.and.Lwrite) WRITE (out,230)                      &
        ...
        DO i=1,nFfiles(ng)
          DO ifile=1,FRC(i,ng)%Nfiles
            ...
            IF (.not.find_file(ng, out, fname, 'FRCNAME')) THEN
              IF (FoundError(exit_flag, NoError, 7865, MyFile))         &
     &          RETURN
            ELSE
              IF (ifile.eq.1) THEN
          ...
          IF (CLM_FILE(ng)) THEN
            DO ifile=1,CLM(i,ng)%Nfiles
              ...
              IF (.not.find_file(ng, out, fname, 'CLMNAME')) THEN
                IF (FoundError(exit_flag, NoError, 7883, MyFile))       &
     &            RETURN
              ELSE
                IF (ifile.eq.1) THEN
                  IF (Master.and.Lwrite) WRITE (out,310)                &
...
!
          ...
          DO i=1,nBCfiles(ng)
            DO ifile=1,BRY(i,ng)%Nfiles
              ...
              IF (.not.find_file(ng, out, fname, 'BRYNAME')) THEN
                IF (FoundError(exit_flag, NoError, 7944, MyFile))       &
     &            RETURN
              ELSE
                IF (ifile.eq.1) THEN
                  IF (Master.and.Lwrite) WRITE (out,310)                &
     ...
     &                                         TRIM(fname)
        ...
        IF (.not.find_file(ng, out, fname, 'VARNAME')) THEN
          IF (FoundError(exit_flag, NoError, 8018, MyFile)) RETURN
        ELSE
          IF (Master.and.Lwrite) WRITE (out,230)                        &
