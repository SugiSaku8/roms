...
!
     ...
     &                        TRIM(ncname))
          IF (FoundError(status, nf90_noerr, 241, MyFile)) THEN
...
!
     ...
     &                        'netCDF-3 64bit offset file')
          IF (FoundError(status, nf90_noerr, 260, MyFile)) THEN
...
!
     ...
     &                        TRIM(type))
          IF (FoundError(status, nf90_noerr, 276, MyFile)) THEN
...
!
     ...
     &                        TRIM(title))
          IF (FoundError(status, nf90_noerr, 375, MyFile)) THEN
     ...
     &                        TRIM(varname))
          IF (FoundError(status, nf90_noerr, 385, MyFile)) THEN
     ...
     &                            TRIM(HIS(ng)%name))
            END IF
            IF (FoundError(status, nf90_noerr, 436, MyFile)) THEN
     ...
     &                        TRIM(INI(ng)%name))
          IF (FoundError(status, nf90_noerr, 577, MyFile)) THEN
     ...
     &                            string(1:lstr))
              IF (FoundError(status, nf90_noerr, 788, MyFile)) THEN
     ...
     &                        TRIM(Iname))
          IF (FoundError(status, nf90_noerr, 842, MyFile)) THEN
...
!
        IF (exit_flag.eq.NoError) THEN
          ...
          IF (FoundError(status, nf90_noerr, 930, MyFile)) THEN
...
!
     ...
     &                        TRIM(svn_url))
          IF (FoundError(status, nf90_noerr, 982, MyFile)) THEN
...
!
     ...
     &                        TRIM(Hfile))
          IF (FoundError(status, nf90_noerr, 1033, MyFile)) THEN
...
!
     ...
     &                        TRIM(my_cpu))
          IF (FoundError(status, nf90_noerr, 1058, MyFile)) THEN
     ...
     &                        TRIM(my_fc))
          IF (FoundError(status, nf90_noerr, 1079, MyFile)) THEN
     ...
     &                        my_fflags(1:lstr))
          IF (FoundError(status, nf90_noerr, 1092, MyFile)) THEN
...
!
     ...
     &                        TRIM(history))
          IF (FoundError(status, nf90_noerr, 1114, MyFile)) THEN
...
!
     ...
     &                        TRIM(Coptions(1:lstr)))
          IF (FoundError(status, nf90_noerr, 1163, MyFile)) THEN
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1223, MyFile)) RETURN
      Vinfo( 1)='dstart'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1232, MyFile)) RETURN
      Vinfo( 1)='nHIS'
      Vinfo( 2)='number of time-steps between history records'
      status=def_var(ng, model, ncid, varid, nf90_int,                  &
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1280, MyFile)) RETURN
      Vinfo( 1)='ndefHIS'
      Vinfo( 2)=                                                        &
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1288, MyFile)) RETURN
      Vinfo( 1)='nRST'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1334, MyFile)) RETURN
      Vinfo( 1)='nAVG'
      Vinfo( 2)='number of time-steps between time-averaged records'
      status=def_var(ng, model, ncid, varid, nf90_int,                  &
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1341, MyFile)) RETURN
      Vinfo( 1)='ndefAVG'
      Vinfo( 2)=                                                        &
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1528, MyFile)) RETURN
      Vinfo( 1)='Fgamma'
      Vinfo( 2)='Power-law shape barotropic filter parameter'
      status=def_var(ng, model, ncid, varid, NF_TOUT,                   &
     &               1, (/0/), Aval, Vinfo, ncname,                     &
...
!
     ...
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1548, MyFile)) RETURN
      Vinfo( 1)='nl_visc2'
      Vinfo( 2)='nonlinear model Laplacian mixing coefficient '//       &
     &          'for momentum'
      Vinfo( 3)='meter2 second-1'
     ...
     &               1, (/trcdim/), Aval, Vinfo, ncname,                &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1710, MyFile)) RETURN
!
...
!
     ...
     &               1, (/trcdim/), Aval, Vinfo, ncname,                &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1722, MyFile)) RETURN
      Vinfo( 1)='Akv_bak'
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 1822, MyFile)) RETURN
      Vinfo( 1)='Zos'
...
!
     ...
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2006, MyFile)) RETURN
        Vinfo( 1)='FSobc_out'
     ...
     &                 1, (/brydim/), Aval, Vinfo, ncname,              &
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2014, MyFile)) RETURN
        Vinfo( 1)='M2obc_in'
     ...
     &                 1, (/brydim/), Aval, Vinfo, ncname,              &
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2022, MyFile)) RETURN
        Vinfo( 1)='M2obc_out'
     ...
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2039, MyFile)) RETURN
        Vinfo( 1)='Tobc_out'
     ...
     &                 2, tbrydim, Aval, Vinfo, ncname,                 &
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2047, MyFile)) RETURN
        Vinfo( 1)='M3obc_in'
     ...
     &                 1, (/brydim/), Aval, Vinfo, ncname,              &
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2055, MyFile)) RETURN
        Vinfo( 1)='M3obc_out'
     ...
     &                 1, (/brydim/), Aval, Vinfo, ncname,              &
     &                 SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 2063, MyFile)) RETURN
      END IF
!
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2096, MyFile)) RETURN
      Vinfo( 1)='Tcoef'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2111, MyFile)) RETURN
!
!  Various parameters.
!
!
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2141, MyFile)) RETURN
!
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2154, MyFile)) RETURN
      Vinfo( 1)='LwSrc'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2163, MyFile)) RETURN
!
...
!
     ...
     &               1, (/trcdim/), Aval, Vinfo, ncname,                &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2177, MyFile)) RETURN
!
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2189, MyFile)) RETURN
      Vinfo( 1)='Lm2CLM'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2198, MyFile)) RETURN
      Vinfo( 1)='Lm3CLM'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2208, MyFile)) RETURN
      Vinfo( 1)='LtracerCLM'
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2229, MyFile)) RETURN
!
      Vinfo( 1)='LnudgeM3CLM'
...
!
     ...
     &               1, (/trcdim/), Aval, Vinfo, ncname,                &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 2248, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3004, MyFile)) RETURN
      Vinfo( 1)='el'
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3012, MyFile)) RETURN
!
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3022, MyFile)) RETURN
      Vinfo( 1)='Vstretching'
      Vinfo( 2)='vertical terrain-following stretching function'
      status=def_var(ng, model, ncid, varid, nf90_int,                  &
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3029, MyFile)) RETURN
      Vinfo( 1)='theta_s'
      Vinfo( 2)='S-coordinate surface control parameter'
      status=def_var(ng, model, ncid, varid, NF_TOUT,                   &
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3036, MyFile)) RETURN
      Vinfo( 1)='theta_b'
      Vinfo( 2)='S-coordinate bottom control parameter'
      status=def_var(ng, model, ncid, varid, NF_TOUT,                   &
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3043, MyFile)) RETURN
      Vinfo( 1)='Tcline'
...
!
     ...
     &               1, (/0/), Aval, Vinfo, ncname,                     &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3067, MyFile)) RETURN
!
...
!
     ...
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3116, MyFile)) RETURN
!
...
!
     ...
     &               1, (/srdim/), Aval, Vinfo, ncname,                 &
     &               SetParAccess = .FALSE.)
      IF (FoundError(exit_flag, NoError, 3129, MyFile)) RETURN
!
...
!
     ...
     &               SetParAccess = .FALSE.)
        IF (FoundError(exit_flag, NoError, 3155, MyFile)) RETURN
      END IF
!
...
!
     ...
     &                   1, (/stadim/), Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 3209, MyFile)) RETURN
        ELSE
          status=def_var(ng, model, ncid, varid, NF_TYPE,               &
...
!
        IF (spherical) THEN
          ...
          IF (ncid.eq.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     1, (/stadim/), Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3269, MyFile)) RETURN
          ELSE
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, t2dgrd, Aval, Vinfo, ncname)
...
!
          ...
          IF (ncid.eq.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     1, (/stadim/), Aval, Vinfo,  ncname)
            IF (FoundError(exit_flag, NoError, 3284, MyFile)) RETURN
          ELSE
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, t2dgrd, Aval, Vinfo, ncname)
...
!
          ...
          IF (ncid.eq.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     1, (/stadim/), Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3314, MyFile)) RETURN
          ELSE
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, t2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3318, MyFile)) RETURN
          END IF
        END IF
!
     ...
     &                     2, u2dgrd, Aval, Vinfo, ncname)
        ...
        ELSE
          ...
          IF (ncid.ne.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, u2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3355, MyFile)) RETURN
          END IF
!
          ...
          IF (ncid.ne.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, u2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3366, MyFile)) RETURN
          END IF
        END IF
!
...
!
          ...
          IF (ncid.ne.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, v2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3414, MyFile)) RETURN
          END IF
        END IF
!
...
!
     ...
     &                     2, p2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3429, MyFile)) RETURN
          END IF
!
          ...
          IF (ncid.ne.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, p2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3440, MyFile)) RETURN
          END IF
        ELSE
...
!
          ...
          IF (ncid.ne.STA(ng)%ncid) THEN
            status=def_var(ng, model, ncid, varid, NF_TYPE,             &
     &                     2, p2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 3462, MyFile)) RETURN
          END IF
        END IF
!
...
!
        IF (ncid.ne.STA(ng)%ncid) THEN
          ...
          status=def_var(ng, model, ncid, varid, NF_TYPE,               &
     &                   2, t2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 3501, MyFile)) RETURN
!
          ...
          status=def_var(ng, model, ncid, varid, NF_TYPE,               &
     &                   2, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 3512, MyFile)) RETURN
!
          ...
          status=def_var(ng, model, ncid, varid, NF_TYPE,               &
     &                   2, v2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 3523, MyFile)) RETURN
!
          ...
          status=def_var(ng, model, ncid, varid, NF_TYPE,               &
     &                   2, p2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 3534, MyFile)) RETURN
        END IF
      END IF
!
  10  FORMAT (i0,'x',i0)
