...
!
      ...
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Utility/check_multifile.F"//", multifile_info_s1d"
!
      SourceFile=MyFile
!
!-----------------------------------------------------------------------
...
!
      ...
      DO ifile=1,Nfiles
        ...
        foundit=check_file(ng, model, Nfiles, ncname, S(ng)%IOtype,     &
...
!
     ...
     &                          'first ', Tmin_code, F_code
            END IF
            exit_flag=4
            IF (FoundError(exit_flag, NoError, 328, MyFile)) RETURN
          END IF
        ELSE
...
!
      ...
      character (len=256) :: ncname
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Utility/check_multifile.F"//", multifile_info_s2d"
!
      SourceFile=MyFile
!
!-----------------------------------------------------------------------
...
!
      DO i=1,InpFiles(ng)
        ...
        DO ifile=1,Nfiles
          ...
          foundit=check_file(ng, model, Nfiles, ncname, S(i,ng)%IOtype, &
     &                       Tmin, Tmax, Tscale, Lcheck)
...
!
      FUNCTION check_file (ng, model, Nfiles, ncname, IOtype,           &
     &                     Tmin, Tmax, Tscale, Lcheck) RESULT (foundit)
!
!=======================================================================
!                                                                      !
...
!     IOtype       I/O processing library (integer):                   !
!                     io_nf90    Standard NetCDF libray                !
!                     io_pio     Parallel I/O library                  !
!                                                                      !
!  On Output:                                                          !
!                                                                      !
...
!
      integer, intent(in) :: ng, model, Nfiles, IOtype
!
...
!
      ...
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Utility/check_multifile.F"//", check_file"
!
      SourceFile=MyFile
!
!------------------------------------------------------------------------
...
!
      ...
      END SELECT
      IF (FoundError(exit_flag, NoError, 629, MyFile)) THEN
...
!
     ...
     &                         ncid = ncid)
      END SELECT
      IF (FoundError(exit_flag, NoError, 646, MyFile)) RETURN
!
...
!
     ...
     &                          start = (/1/),                          &
     &                          total = (/1/))
          IF (FoundError(exit_flag, NoError, 741, MyFile)) RETURN
!
          CALL netcdf_get_time (ng, model, ncname, TvarName,            &
