...
!
      DEFINE : IF (ldef) THEN
        ...
        IF (FoundError(exit_flag, NoError, 194, MyFile)) THEN
...
!
     ...
     &                   Nslice, DimIDs(34))
          IF (FoundError(exit_flag, NoError, 291, MyFile)) RETURN
        END IF
     ...
     &                 nf90_unlimited, DimIDs(12))
        IF (FoundError(exit_flag, NoError, 362, MyFile)) RETURN
        recdim=DimIDs(12)
!
...
!
     ...
     &                 SetParAccess = .TRUE.)
        IF (FoundError(exit_flag, NoError, 561, MyFile)) RETURN
!
...
!
     ...
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 691, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 731, MyFile)) RETURN
        END IF
!
!  Define free-surface.
!
...
!
     ...
     &                   NF_FOUT, nvd3, t2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1029, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, u3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1069, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, u3dzgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1106, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, t3dzgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1241, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, p3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1299, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, p3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1318, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, p3dzgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1358, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, w3dgrd, Aval, Vinfo, ncname,    &
     &                   SetFillVal = .FALSE.)
          IF (FoundError(exit_flag, NoError, 1610, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                     nvd3, t2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 1871, MyFile)) RETURN
          END IF
        END DO
!
!  Define E-P flux.
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 2070, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, v2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 2127, MyFile)) RETURN
        END IF
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2192, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2214, MyFile)) THEN
...
!
     ...
     &                         ncid = HIS(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 2223, MyFile)) RETURN
!
...
!  NetCDF and activate switches for history variables. Get variable IDs.
!
        CALL bbl_def_nf90 (ng, model, ldef, Hout, HIS)
