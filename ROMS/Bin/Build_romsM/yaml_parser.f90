...
!     have:                                                            !
!                                                                      !
!       state variables: [sea_surface_height_above_geopotential_datum, !
...
!
      CALL self%create ()
      IF (yaml_Error(yaml_ErrFlag, NoErr, 354, MyFile)) THEN
        status=yaml_ErrFlag
        RETURN
      END IF
!
...
!
     ...
     &      STATUS='old', IOSTAT=io_err, IOMSG=io_errmsg)
      IF (io_err.ne.0) THEN
        yaml_ErrFlag=5
        IF (yaml_Error(yaml_ErrFlag, NoErr, 410, MyFile)) THEN
...
!
        SELECT CASE (status)
          CASE (-1)       ! error condition during reading
            yaml_ErrFlag=5
            IF (yaml_Error(yaml_ErrFlag, NoErr, 433, MyFile)) THEN
...
!
        IF (Nblanks.gt.0) THEN
          IF (MOD(Nblanks, self%indent).ne.0) THEN
            yaml_ErrFlag=2
            IF (yaml_Error(yaml_ErrFlag, NoErr, 465, MyFile)) THEN
...
!
      ...
      ELSE
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 779, MyFile)) THEN
...
!
        SELECT CASE (status)
          CASE (-1)       ! error condition during reading
            yaml_ErrFlag=4
            IF (yaml_Error(yaml_ErrFlag, NoErr, 1028, MyFile)) THEN
              IF (yaml_Master) WRITE (yaml_stdout,10) self%filename,    &
     &                                                TRIM(line)
              RETURN
            END IF
...
!
      ...
      IF (Icolon.eq.0) THEN
        ...
        END IF
        IF (yaml_Error(yaml_ErrFlag, NoErr, 1454, MyFile)) RETURN
      END IF
!
...
!
     ...
     &           V(n)%vector(i)
            IF (yaml_Error(status, NoErr, 1798, MyFile)) THEN
              yaml_ErrFlag=5
...
!
      DO n=1,Nitems
        IF (S(n)%has_vector) THEN
          ...
          DO i=1,Nvalues
            READ (S(n)%vector(i)%value, * ,IOSTAT=status, IOMSG=msg)    &
     &           V(n)%vector(i)
            IF (yaml_Error(status, NoErr, 1957, MyFile)) THEN
              yaml_ErrFlag=5
              IF (yaml_Master) WRITE (yaml_stdout,10) TRIM(keystring),  &
     &                                            S(n)%vector(i)%value, &
     &                                                TRIM(msg)
     ...
     &         V(n)%vector(i)
          IF (yaml_Error(status, NoErr, 1968, MyFile)) THEN
            yaml_ErrFlag=5
            IF (yaml_Master) WRITE (yaml_stdout,10) TRIM(keystring),    &
     &                                              S(n)%value,         &
     &                                              TRIM(msg)
...
!
      IF (Nvalues.gt.1) THEN
        status=7
        yaml_ErrFlag=status
        IF (yaml_Error(status, NoErr, 2112, MyFile)) THEN
...
!
      ELSE
        ...
        IF (yaml_Error(status, NoErr, 2122, MyFile)) THEN
          yaml_ErrFlag=5
...
!
      IF (SIZE(value, DIM=1).lt.Nvalues) THEN
        status=7
        yaml_ErrFlag=status
        IF (yaml_Error(status, NoErr, 2193, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            SIZE(value, DIM=1),   &
     &                                            self%filename
          RETURN
...
!
      DO i=1,Nvalues
        ...
        IF (yaml_Error(status, NoErr, 2205, MyFile)) THEN
          yaml_ErrFlag=5
          IF (yaml_Master) WRITE (yaml_stdout,20) TRIM(keystring),      &
     &                                            S(i)%value, TRIM(msg)
          RETURN
...
!
      IF (Nvalues.gt.1) THEN
        status=7
        yaml_ErrFlag=status
        IF (yaml_Error(status, NoErr, 2274, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            self%filename
          RETURN
        END IF
...
!
      IF (SIZE(value, DIM=1).lt.Nvalues) THEN
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 2349, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            SIZE(value, DIM=1),   &
     &                                            self%filename
          RETURN
...
!
      IF (Nvalues.gt.1) THEN
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 2429, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            self%filename
          RETURN
        END IF
...
!
      ELSE
        ...
        IF (yaml_Error(status, NoErr, 2441, MyFile)) THEN
          yaml_ErrFlag=5
          IF (yaml_Master) WRITE (yaml_stdout,20) TRIM(keystring),      &
     &                                            S(1)%value, TRIM(msg)
          RETURN
...
!
      IF (SIZE(value, DIM=1).lt.Nvalues) THEN
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 2512, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            SIZE(value, DIM=1),   &
     &                                            self%filename
          RETURN
...
!
      DO i=1,Nvalues
        ...
        IF (yaml_Error(status, NoErr, 2526, MyFile)) THEN
          yaml_ErrFlag=5
          IF (yaml_Master) WRITE (yaml_stdout,20) TRIM(keystring),      &
     &                                            S(i)%value, TRIM(msg)
          RETURN
...
!
      IF (Nvalues.gt.1) THEN
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 2596, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            self%filename
          RETURN
        END IF
...
!
      IF (SIZE(value, DIM=1).lt.Nvalues) THEN
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 2667, MyFile)) THEN
          IF (yaml_Master) WRITE (yaml_stdout,10) keystring, Nvalues,   &
     &                                            SIZE(value, DIM=1),   &
     &                                            self%filename
          RETURN
